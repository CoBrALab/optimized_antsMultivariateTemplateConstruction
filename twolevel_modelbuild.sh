...
# ARG_OPTIONAL_SINGLE([secondlevel-final-target-mask],[],[Second-level Final target mask],[])
...
print_help()
{
  ...
  printf '\t%s\n' "--secondlevel-final-target-mask: Second-level Final target mask (no default)"
  ...
  do
    ...
    case "$_key" in
      ...
      --firstlevel-starting-target)
        test $# -lt 2 && die "Missing value for the optional argument '$_key'." 1
        _arg_firstlevel_starting_target="$2"
        shift
        ;;
      --firstlevel-starting-target=*)
...
${__dir}/modelbuild.sh ${_arg_debug:+--debug} \
  --skip-file-checks \
  --job-predepend "twolevel_${__datetime}_" \
  ${_arg_leftovers[@]+"${_arg_leftovers[@]}"} \
