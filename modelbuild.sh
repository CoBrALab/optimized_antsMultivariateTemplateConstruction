...
# ARG_OPTIONAL_BOOLEAN([average-norm],[],[Whether to normalize each image by their mean before evaluating average.],[on])
# ARG_OPTIONAL_SINGLE([trim-percent],[],[Percentage to cut off if using trimmed_mean],[0.15])
...
print_help()
{
  ...
  printf '\t%s\n' "--average-norm, --no-average-norm: Whether to normalize each image by their mean before evaluating average. (on by default)"
  printf '\t%s\n' "--trim-percent: Percentage to cut off if using trimmed_mean (default: '0.15')"
      ...
      if [[ ! -s ${_arg_output_dir}/${reg_type}/${i}/average/template_sharpen_shapeupdate.nii.gz ]]; then
        ...
        case ${_arg_average_type} in
          mean)
            ...
            ;;
          median)
            echo ${SCRIPT_DIR}/modelbuild_averager.py -o ${_arg_output_dir}/${reg_type}/${i}/average/template.nii.gz ${_normalize} --image_type image --method ${_arg_average_type} --trim_percent ${_arg_trim_percent} \
        ...
        if [[ ${reg_type} == "nlin" || ${reg_type} == "nlin-only"  ]]; then
          ...
          if [[ ${_arg_average_type} == "mean" ]]; then
