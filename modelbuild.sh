...
print_help()
{
  ...
  printf 'Usage: %s [-h|--help] [--output-dir <arg>] [--gradient-step <arg>] [--starting-target <arg>] [--iterations <arg>] [--(no-)fast] [--average-type <AVERAGE>] [--masks <arg>] [--sharpen-type <SHARPEN>] [--stages <arg>] <inputs-1> [<inputs-2>] ... [<inputs-n>] ...\n' "$0"
...
if [[ ${_arg_starting_target} == "none" ]]; then
  if [[ ! -s ${_arg_output_dir}/startingtarget.nii.gz ]]; then
    case ${_arg_average_type} in
      mean)
        AverageImages 3 ${_arg_output_dir}/startingtarget.nii.gz 0 "${_arg_inputs[@]}"
        ;;
      normmean)
        AverageImages 3 ${_arg_output_dir}/startingtarget.nii.gz 2 "${_arg_inputs[@]}"
        ;;
      median)
        printf '%s\n' "${_arg_inputs[@]}" > ${_arg_output_dir}/medianlist.txt
        ImageSetStatistics 3 ${_arg_output_dir}/medianlist.txt ${_arg_output_dir}/startingtarget.nii.gz 0
...
for reg_type in "${_arg_stages[@]}"; do
  ...
  while ((i < _arg_iterations)); do
    ...
    if [[ ! -s ${_arg_output_dir}/${reg_type}/${i}/average/template_shapeupdate.nii.gz ]]; then
      ...
      done | tee ${_arg_output_dir}/jobs/${reg_type}_${i}_reg
      ...
      if [[ ! -s ${_arg_output_dir}/${reg_type}/${i}/average/template_sharpen_shapeupdate.nii.gz ]]; then
        ...
        case ${_arg_average_type} in
          mean)
            echo AverageImages 3 ${_arg_output_dir}/${reg_type}/${i}/average/template.nii.gz \
              1 $(for subject in "${_arg_inputs[@]}"; do echo -n "${_arg_output_dir}/${reg_type}/${i}/resample/$(basename ${subject}) "; done) \
              | tee -a ${_arg_output_dir}/jobs/${reg_type}_${i}_shapeupdate
            ;;
          normmean)
            echo AverageImages 3 ${_arg_output_dir}/${reg_type}/${i}/average/template.nii.gz \
              2 $(for subject in "${_arg_inputs[@]}"; do echo -n "${_arg_output_dir}/${reg_type}/${i}/resample/$(basename ${subject}) "; done) \
              | tee -a ${_arg_output_dir}/jobs/${reg_type}_${i}_shapeupdate
            ;;
          median)
            for subject in "${_arg_inputs[@]}"; do echo -n "${_arg_output_dir}/${reg_type}/${i}/resample/$(basename ${subject}) "; done > ${_arg_output_dir}/medianlist.txt
        ...
        echo ${AVERAGE_AFFINE_PROGRAM} 3 ${_arg_output_dir}/${reg_type}/${i}/average/affine.mat \
          $(for subject in "${_arg_inputs[@]}"; do echo -n "${_arg_output_dir}/${reg_type}/${i}/transforms/$(basename ${subject} .nii.gz)_0GenericAffine.mat "; done) \
          | tee -a ${_arg_output_dir}/jobs/${reg_type}_${i}_shapeupdate

        if [[ ${reg_type} == "nlin" ]]; then
          echo AverageImages 3 ${_arg_output_dir}/${reg_type}/${i}/average/warp.nii.gz \
            0 $(for subject in "${_arg_inputs[@]}"; do echo -n "${_arg_output_dir}/${reg_type}/${i}/transforms/$(basename ${subject} .nii.gz)_1Warp.nii.gz "; done) \
            | tee -a ${_arg_output_dir}/jobs/${reg_type}_${i}_shapeupdate
