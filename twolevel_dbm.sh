...
# ARG_OPTIONAL_SINGLE([jacobian-smooth],[],[Comma separated list of smoothing Gaussian FWHMs, append "vox" for voxels, "mm" for millimeters],[4vox])
...
print_help()
{
  ...
  printf 'Usage: %s [-h|--help] [--output-dir <arg>] [--jacobian-smooth <arg>] [--walltime <arg>] [--target-space <SPACE>] [--(no-)debug] [--(no-)dry-run] <inputs> ... \n' "$0"
...
if [[ ${_arg_dry_run} == "off" ]]; then
  ...
  --target-space ${_arg_target_space} \
...
while read -r subject_scans; do
  ...
  if [[ $(wc -l < ${_arg_output_dir}/firstlevel/subject_${i}/input_files.txt) -gt 1 ]]; then
    ...
    if [[ ${_arg_dry_run} == "off" ]]; then
      ${__dir}/dbm.sh ${_arg_debug:+--debug} \
        ${_arg_leftovers[@]+"${_arg_leftovers[@]}"} \
        --jobname-prefix "dbm_twolevel_${__datetime}_subject_${i}_" \
        --jacobian-smooth "${_arg_jacobian_smooth}" \
        --output-dir ${_arg_output_dir}/firstlevel/subject_${i} \
        ${_arg_output_dir}/firstlevel/subject_${i}/input_files.txt
    fi

    for scan in "${scans[@]}"; do
      info "Resampling Jacobian Determinants"
      if [[ ${_arg_target_space} == "unbiased" ]]; then
        # Resample within-subject into unbiased space
        ...
        if [[ ! -s ${_arg_output_dir}/secondlevel/resampled-dbm/jacobian/full/final-target/$(basename ${scan} | extension_strip).nii.gz ]]; then
          echo "antsApplyTransforms -d 3 --verbose \
            ...
            -i ${_arg_output_dir}/firstlevel/subject_${i}/dbm/jacobian/full/$(basename ${scan} | extension_strip).nii.gz \
            -t ${_arg_output_dir}/secondlevel/final-target/to_target_1Warp.nii.gz \
            -t ${_arg_output_dir}/secondlevel/final-target/to_target_0GenericAffine.mat \
            -t ${_arg_output_dir}/secondlevel/final/transforms/subject_${i}_1Warp.nii.gz \
            -t ${_arg_output_dir}/secondlevel/final/transforms/subject_${i}_0GenericAffine.mat \
            -o ${_arg_output_dir}/secondlevel/resampled-dbm/jacobian/full/final-target/$(basename ${scan} | extension_strip).nii.gz"
        fi
        if [[ ! -s ${_arg_output_dir}/secondlevel/resampled-dbm/jacobian/relative/final-target/$(basename ${scan} | extension_strip).nii.gz ]]; then
          echo "antsApplyTransforms -d 3 --verbose \
            -r ${_arg_output_dir}/secondlevel/final-target/final_target.nii.gz \
            -i ${_arg_output_dir}/firstlevel/subject_${i}/dbm/jacobian/relative/$(basename ${scan} | extension_strip).nii.gz \
            -t ${_arg_output_dir}/secondlevel/final-target/to_target_1Warp.nii.gz \
            ...
            -t ${_arg_output_dir}/secondlevel/final/transforms/subject_${i}_1Warp.nii.gz \
            -t ${_arg_output_dir}/secondlevel/final/transforms/subject_${i}_0GenericAffine.mat \
            -o ${_arg_output_dir}/secondlevel/resampled-dbm/jacobian/relative/final-target/$(basename ${scan} | extension_strip).nii.gz"
        fi
      fi
    ...
    for scan in "${scans[@]}"; do
      ...
      if [[ ${_arg_target_space} == "unbiased" ]]; then
        # Generate Overall Jacobian Determinants
        ...
        if [[ ! -s ${_arg_output_dir}/secondlevel/overall-dbm/jacobian/full/final-target/$(basename ${scan} | extension_strip).nii.gz ]]; then
          echo "ImageMath 3 ${_arg_output_dir}/secondlevel/overall-dbm/jacobian/full/final-target/$(basename ${scan} | extension_strip).nii.gz + \
            ${_arg_output_dir}/secondlevel/dbm/jacobian/final-target/full/subject_${i}.nii.gz \
            ${_arg_output_dir}/secondlevel/resampled-dbm/jacobian/full/final-target/$(basename ${scan} | extension_strip).nii.gz"
        fi
        if [[ ! -s ${_arg_output_dir}/secondlevel/overall-dbm/jacobian/relative/final-target/$(basename ${scan} | extension_strip).nii.gz ]]; then
          echo "ImageMath 3 ${_arg_output_dir}/secondlevel/overall-dbm/jacobian/relative/final-target/$(basename ${scan} | extension_strip).nii.gz + \
            ${_arg_output_dir}/secondlevel/dbm/jacobian/final-target/relative/subject_${i}.nii.gz \
            ${_arg_output_dir}/secondlevel/resampled-dbm/jacobian/relative/final-target/$(basename ${scan} | extension_strip).nii.gz"
        fi
      fi
    done >>${_arg_output_dir}/secondlevel/dbm/jobs/${__datetime}/overall_jacobian

    for scan in "${scans[@]}"; do
      # Smooth Jacobian Determinant Files
      ...
      for fwhm in "${_arg_jacobian_smooth_list[@]}"; do
        ...
        if [[ ${_arg_target_space} == "unbiased" ]]; then
          ...
          if [[ ! -s ${_arg_output_dir}/secondlevel/resampled-dbm/jacobian/full/final-target/smooth/$(basename ${scan} | extension_strip)_fwhm_${fwhm}.nii.gz ]]; then
            echo "SmoothImage 3 \
              ${_arg_output_dir}/secondlevel/resampled-dbm/jacobian/full/final-target/$(basename ${scan} | extension_strip).nii.gz \
              ${sigma_num} \
              ${_arg_output_dir}/secondlevel/resampled-dbm/jacobian/full/final-target/smooth/$(basename ${scan} | extension_strip)_fwhm_${fwhm}.nii.gz ${fwhm_type} 0"
          fi
          if [[ ! -s ${_arg_output_dir}/secondlevel/resampled-dbm/jacobian/relative/final-target/smooth/$(basename ${scan} | extension_strip)_fwhm_${fwhm}.nii.gz ]]; then
            echo "SmoothImage 3 \
              ${_arg_output_dir}/secondlevel/resampled-dbm/jacobian/relative/final-target/$(basename ${scan} | extension_strip).nii.gz \
              ${sigma_num} \
              ${_arg_output_dir}/secondlevel/resampled-dbm/jacobian/relative/final-target/smooth/$(basename ${scan} | extension_strip)_fwhm_${fwhm}.nii.gz ${fwhm_type} 0"
          fi
